...
export default function LibraryPage() {
  ...
  const downloadedNotes = allNotes?.filter(note => 
    downloads?.some(download => download.noteId === note.id)
  ) || [];
  
  // Get user's liked notes
  const likedNotes = allNotes?.filter(note => 
    userLikes?.some(like => like.noteId === note.id)
  ) || [];
